function filterTags(e){e=[parseInt(e.find(":selected").attr("value"))];$.ajax({type:"GET",url:"/Item/GetThemeTags",data:{ids:e},dataType:"json",cache:!1,traditional:!0,success:function(e){$("#checkboxes label").hide();for(var t=0;t<e.length;t++)$("#tag-input-id-"+e[t]).parent().show()},error:function(){alert("Error occured!!")}})}function filterProjects(e){e=[parseInt(e.find(":selected").attr("value"))];$.ajax({type:"GET",url:"/Item/GetTeamProjects",data:{ids:e},dataType:"json",cache:!1,traditional:!0,success:function(e){$("#project-selector option:not(:first)").hide();for(var t=0;t<e.length;t++)$("#project-selector option[value="+e[t]+"]").show()},error:function(){alert("Error occured!!")}})}$(document).ready(function(){$(":checkbox").change(function(){$(this).is(":checked")?$(this).attr("checked",!0):$(this).attr("checked",!1)}),filterTags($("#theme-selector")),filterProjects($("#team-selector")),$("#theme-selector").change(function(){filterTags($(this))}),$("#team-selector").change(function(){filterProjects($(this))})});var expanded=!1;function showCheckboxes(){var e;0==$("#tag-selector select").prop("disabled")&&(e=document.getElementById("checkboxes"),expanded=expanded?!(e.style.display="none"):(e.style.display="block",!0))}function addKeyInsightField(e){var t=$(e).parent().find(".key-insight-field").length;const a=$(e).parent();a.append(`<div class="key-insight-field row">
                        ...
                        <div class="col-2"><button onclick="removeField(this, 'keyInsight')">Delete</button></div>
                            ...
                            </div>`),0==t&&(e.innerHTML="Suggest another tag"),event.preventDefault()}function showField(e,t){$("#"+t).show(),$(e).hide(),event.preventDefault()}function removeField(e,t){$(e).parent().parent().remove(),"suggestedTag"==t&&0==$(".suggested-tag-field").length&&$("#add-suggested-tag-button").html("Suggest a tag"),event.preventDefault()}function hideField(e,t){$("#"+e+" input").val(""),$("#"+e).hide(),$("#"+t).show(),event.preventDefault()}function handleFiles(e){for(var t=e.files,a=new FormData,r=0;r<t.length;r++)a.append(t[r].name,t[r]);$.ajax({url:"/Item/PostFiles",cache:!1,contentType:!1,processData:!1,data:a,type:"post",success:function(e){var t=$("#file-names").val()+e;if($("#file-names").val(t),e){let t=e.split(",");t.pop();let a="";for(let e=0;e<t.length;e++){var r="/Item/GetDownloadedFile/?filePath=\\files\\documents\\items\\temp\\"+t[e];a+=`<div class="file"><a href="${r}"><h2>${t[e]}</h2></a><button onclick="removeFile(this)">Delete</button><br><br></div>`}$("#files-list").append(a)}},error:function(e){console.log(e),alert("An error occurred!!!")}}),event.preventDefault()}function removeFile(e){const t=$(e).parent().find("h2").first().text();$.ajax({url:"/Item/DeleteFiles",contentType:"application/json; charset=utf-8",data:{name:t},type:"GET",cache:!1,success:function(){var e=$("#file-names").val().replace(t+",","");$("#file-names").val(e),console.log($("#file-names").val())},error:function(e){console.log(e),alert("An error occurred!!!")}}),$(e).parent().remove(),event.preventDefault()}function filterProfileItems(e){$(".item").hide(),$(`.item[data-status='${e}']`).show(),$(".selected").removeClass("selected"),$(this).addClass("selected")}function getTeams(){let s=[];return $(".team").each(function(e,t){let a=[];const r=$(t).find(".team-id").first().val();var i=$(t).find(".team-name").first().val();$(t).find(".project").each(function(e,t){t={Id:$(t).find(".project-id").first().val(),Name:$(t).find(".project-name").first().val(),TeamId:r};a.push(t)});i={Id:r,Name:i,Projects:a};s.push(i)}),s}function saveTempTeams(e,r){e=JSON.stringify(e);$.ajax({type:"POST",url:"/Team/SaveTeamsState",traditional:!0,data:e,contentType:"application/json; charset=utf-8",dataType:"json",success:function(t){var a=$("#project-teamId-modal-input");$("#project-teamId-modal-input option:gt(0)").remove();for(let e=0;e<t.length;e++)a.append($("<option></option>").attr("value",t[e].value).text(t[e].text));$("#project-teamId-modal-input").val(r)},error:function(e){console.log(e),alert("An error occurred!!!")}})}function updateProjects(e){const t=e.closest(".project-modal");var a=t.find("#project-id-modal-input").first().val();const r=t.find("#project-teamId-modal-input").find(":selected").attr("value");var i=t.find("#project-oldTeamId-modal-input").val(),s=t.find("#project-name-modal-input").first().val(),e=e.val();$.ajax({type:"GET",url:"/Project/UpdateProject",data:{id:a,projectName:s,teamId:r,oldTeamId:i,actionName:e},contentType:"application/json; charset=utf-8",success:function(e){updateTeamProjects(r)},error:function(e){console.log(e),alert("An error occurred!!!")}})}function updateTeamProjects(){window.location.replace("../team?redirect=True")}function openProjectModal(e){$("#project-delete-button").show().prop("disabled",!1),$("#project-submit-button").val("Update");var t=$(e).siblings(".project-name").first().val(),a=$(e).siblings(".project-id").first().val(),r=$(e).closest(".team").find(".team-id").first().val();$(e).siblings("#project-oldTeamId-modal-input").first().val();$("#project-name-modal-input").attr("value",t),$("#project-id-modal-input").attr("value",a),$("#project-teamId-modal-input").val(r),$("#project-oldTeamId-modal-input").val(r),$("#myProjectModal").show(),saveTempTeams(getTeams(),r),event.preventDefault()}function openAddProjectModal(e){console.log("kk"),$("#project-delete-button").hide().prop("disabled",!0),$("#project-submit-button").val("Add");var t=$(e).closest(".team").find(".team-id").first().val();$(e).siblings("#project-oldTeamId-modal-input").first().val();$("#project-teamId-modal-input").val(t),$("#project-oldTeamId-modal-input").val(t),$("#project-id-modal-input").attr("value",0),$("#project-name-modal-input").attr("value",""),$("#myProjectModal").show(),saveTempTeams(getTeams(),t)}$(document).ready(function(){$("#file-upload-box").change(function(){handleFiles(this)}),null!=$("#theme-selector").val()&&0!=$("#theme-selector").val()&&$("#tag-selector select").prop("disabled",!1),$("#team-selector").val()&&$("#project-selector").prop("disabled",!1),$("#team-selector").change(function(){0!=$(this).val()&&($("#project-selector").prop("selectedIndex",0),$("#project-selector").prop("disabled",!1))}),$("#theme-selector").change(function(){0!=$(this).val()&&$("#tag-selector select").prop("disabled",!1)}),$("#status-selector").change(function(){"Rejected"==$(this).val()?$("#comment-input").show():$("#comment-input").hide()})}),$(document).ready(function(){$("#profile-edit-btn").click(function(){event.preventDefault(),$("#profile-save-btn").show(),$("#user-role-input").show(),$("#user-name-input").show(),$("#user-team-selector").show(),$("#user-name-text").hide(),$("#user-team-text").hide(),$("#user-role-text").hide(),$(this).hide()}),$(".selected").first().click()}),$(document).ready(function(){$(".projectModalClose").click(function(){$("#myProjectModal").hide()}),$(".project-modal-btn").click(function(){updateProjects($(this))}),$(document).mouseup(function(e){const t=$("#myProjectModal");t.is(":visible")&&t.is(e.target)&&0===t.has(e.target).length&&(t.hide(),$("#project-name-input").val(""))})});let queryParameters={searchText:searchText="",themes:themes=[],teams:teams=[],projects:projects=[],tags:tags=[],sensitivity:sensitivity=[],approvals:approvals=[],status:status=["Published"],userIds:userIds=[],startDate:startDate="0001-01-01",endDate:endDate="0001-01-01",paginationStartItem:paginationStartItem="0"};const numOfItemsPerPage=2;function updateFilterParameters(e,t){if(queryParameters.themes=[],queryParameters.teams=[],queryParameters.projects=[],queryParameters.tags=[],queryParameters.sensitivity=[],queryParameters.approvals=[],$("#approval-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.approvals.push($(this).data("name"))}),$("#sensitivity-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.sensitivity.push($(this).data("name"))}),$("#tag-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.tags.push($(this).data("name"))}),$("#team-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.teams.push($(this).data("name"))}),$("#theme-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.themes.push($(this).data("name"))}),$("#project-checkbox-filter input:visible:checked").each(function(e,t){queryParameters.projects.push($(this).data("name"))}),$("#start-date-search").length&&(queryParameters.startDate=$("#start-date-search").val()),$("#end-date-search").length)queryParameters.endDate=$("#end-date-search").val();else{let e=new Date;var a=e.getTimezoneOffset();e=new Date(e.getTime()-60*a*1e3),queryParameters.endDate=e.toISOString().split("T")[0]}queryParameters.paginationStartItem=e*numOfItemsPerPage,t&&(queryParameters.status=[],queryParameters.status.push(t)),filterItemList(t)}function filterItemList(s){var e=JSON.stringify(queryParameters);$.ajax({type:"GET",url:"/Item/GetFilteredItems",data:{itemQueryJson:e},success:function(t){let a="";for(let e=0;e<t.items.length;e++)"Draft"==s&&(a+=`<tr>
                ...
                </tr>`),"Published"!=s&&"Rejected"!=s||(a+=`<tr>
                    <td width="33%">${t.items[e].title}</td>
                    <td width="33%">${t.items[e].team}</td>
                    <td width="33%">${t.items[e].abstract}</td>
                    <td class="text-center">
